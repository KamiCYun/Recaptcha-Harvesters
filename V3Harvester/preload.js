...
contextBridge.exposeInMainWorld(
  "harvester",{
    login: () => {
      ipcRenderer.sendSync('synchronous-message', ['login', null])
      },
    
    setproxy: () => {
      proxy = document.getElementById("proxy").value
      if (proxy != ''){
        document.getElementById("CurrentProx").innerHTML = proxy
        ipcRenderer.sendSync('synchronous-message', ['proxy', proxy])
      }

    },

    getproxy: () => {
      document.getElementById("CurrentProx").innerHTML = ipcRenderer.sendSync('synchronous-message', ['getprox', null])
    },

    savetoken: (token) => {
      ipcRenderer.sendSync('synchronous-message', ['token', token])
    }
  }
)
